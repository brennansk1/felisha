"""vLLM adapter — OpenAI-compatible HTTP client.

vLLM (https://github.com/vllm-project/vllm) exposes the same
``/v1/chat/completions`` surface as llama.cpp's server but is tuned for
*multi-tenant* / *concurrent* workloads via PagedAttention. Prefer this
engine when the master loop is fanning out many parallel calls
(e.g. expert panels, batch hypothesis generation).

vLLM's structured-output story uses ``guided_json`` / ``guided_decoding``
extensions — we pass the caller's ``json_schema`` via ``guided_json`` and
fall back to ``response_format={"type": "json_object"}`` otherwise.
"""
...
class VllmAdapter:
    ...
    Best for multi-tenant / concurrent workloads thanks to PagedAttention.
    Single-stream latency is competitive but llama.cpp typically wins
    there; pick vLLM when you have ``N`` concurrent calls to a single GPU.
    """
    ...
    def healthcheck(self) -> bool:
        """vLLM exposes ``/health`` (200 OK when ready)."""
        try:
            client = self._http()
    ...
    def _build_body(
        ...
        *,
        ...
        # vLLM-specific structured-output: ``guided_json`` (preferred) or
        # ``response_format`` for plain JSON.
    ...
    def _http(self) -> Any:
        try:
            ...
            raise EngineNotAvailable("vllm", "httpx not installed") from exc
...
register_engine("vllm", VllmAdapter)
